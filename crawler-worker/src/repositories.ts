...
export class DocumentRepository {
  ...
  async createSeedSite(input: { name: string; baseUrl: string; allowedHosts: string[]; includePatterns?: string[]; excludePatterns?: string[]; maxDepth?: number; maxRequestsPerMinute?: number; maxConcurrency?: number; maxPdfBytes?: number; }): Promise<SeedSite> {
    const result = await this.db.query<SeedRow>(`
      ...
      RETURNING *
    `, [input.name, input.baseUrl, JSON.stringify(input.allowedHosts), JSON.stringify(input.includePatterns ?? []), JSON.stringify(input.excludePatterns ?? []), input.maxDepth ?? 4, input.maxRequestsPerMinute ?? 30, input.maxConcurrency ?? 2, input.maxPdfBytes ?? 104857600]);
  ...
  async createCrawlJob(input: { seedSiteId: string | null; triggerType: CrawlJobRecord['triggerType']; startUrl: string }): Promise<CrawlJobRecord> {
    const result = await this.db.query<CrawlJobRow>(`INSERT INTO crawl_jobs (seed_site_id, trigger_type, start_url, status) VALUES ($1,$2,$3,'queued') RETURNING *`, [input.seedSiteId, input.triggerType, input.startUrl]);
  ...
  async updateCrawlJob(id: string, patch: Partial<Pick<CrawlJobRecord, 'status' | 'pagesFetched' | 'filesDiscovered' | 'filesDownloaded' | 'duplicatesFound' | 'errorsCount' | 'errorSummary'>> & { started?: boolean; finished?: boolean }): Promise<void> {
    await this.db.query(`UPDATE crawl_jobs SET status = COALESCE($2, status), pages_fetched = COALESCE($3, pages_fetched), files_discovered = COALESCE($4, files_discovered), files_downloaded = COALESCE($5, files_downloaded), duplicates_found = COALESCE($6, duplicates_found), errors_count = COALESCE($7, errors_count), error_summary = COALESCE($8, error_summary), started_at = CASE WHEN $9 THEN COALESCE(started_at, now()) ELSE started_at END, finished_at = CASE WHEN $10 THEN now() ELSE finished_at END WHERE id = $1`, [id, patch.status ?? null, patch.pagesFetched ?? null, patch.filesDiscovered ?? null, patch.filesDownloaded ?? null, patch.duplicatesFound ?? null, patch.errorsCount ?? null, patch.errorSummary ?? null, patch.started ?? false, patch.finished ?? false]);
  }
  ...
  async recordCrawlPage(input: { jobId: string; url: string; normalizedUrl: string; statusCode: number | null; contentType: string | null; depth: number; pageTitle: string | null; error: string | null }): Promise<void> {
    await this.db.query(`INSERT INTO crawl_pages (crawl_job_id, url, normalized_url, status_code, content_type, depth, page_title, error) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (crawl_job_id, normalized_url) DO UPDATE SET status_code=EXCLUDED.status_code, content_type=EXCLUDED.content_type, page_title=EXCLUDED.page_title, error=EXCLUDED.error, fetched_at=now()`, [input.jobId, input.url, input.normalizedUrl, input.statusCode, input.contentType, input.depth, input.pageTitle, input.error]);
  }
  ...
  async upsertDiscoveredLink(input: { url: string; normalizedUrl: string; referrerUrl: string | null; sourceHost: string; anchorText: string | null; likelyDocument: boolean; ingestionStatus: string }): Promise<void> {
    await this.db.query(`INSERT INTO discovered_links (url, normalized_url, referrer_url, source_host, anchor_text, likely_document, ingestion_status) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (normalized_url) DO UPDATE SET url=EXCLUDED.url, referrer_url=COALESCE(EXCLUDED.referrer_url, discovered_links.referrer_url), anchor_text=COALESCE(EXCLUDED.anchor_text, discovered_links.anchor_text), likely_document=EXCLUDED.likely_document, ingestion_status=CASE WHEN discovered_links.ingestion_status='downloaded' THEN 'downloaded' ELSE EXCLUDED.ingestion_status END, last_seen_at=now()`, [input.url, input.normalizedUrl, input.referrerUrl, input.sourceHost, input.anchorText, input.likelyDocument, input.ingestionStatus]);
  }
  ...
  async upsertDocumentByHash(input: { sha256: string; storageKey: string; mimeType: string; byteSize: number; preferredTitle: string | null; preferredFilename: string | null; documentNumber: string | null }): Promise<{ document: DocumentRecord; duplicate: boolean }> {
    const result = await this.db.query<DocumentRow>(`
      ...
      ON CONFLICT (sha256) DO UPDATE SET last_seen_at=now(), updated_at=now(), preferred_title=COALESCE(documents.preferred_title, EXCLUDED.preferred_title), preferred_filename=COALESCE(documents.preferred_filename, EXCLUDED.preferred_filename), document_number=COALESCE(documents.document_number, EXCLUDED.document_number), document_number_normalized=COALESCE(documents.document_number_normalized, EXCLUDED.document_number_normalized)
  ...
  async upsertDocumentBlob(input: { documentId: string; sha256: string; content: Buffer }): Promise<void> {
    await this.db.query(`
      INSERT INTO document_blobs (document_id, sha256, byte_size, content)
      VALUES ($1,$2,$3,$4)
      ON CONFLICT (document_id) DO UPDATE SET sha256=EXCLUDED.sha256, byte_size=EXCLUDED.byte_size, content=EXCLUDED.content, updated_at=now()
    `, [input.documentId, input.sha256, input.content.length, input.content]);
    await this.db.query(`UPDATE documents SET storage_backend='hybrid', updated_at=now() WHERE id=$1`, [input.documentId]);
  }

  async getDocumentBlob(documentId: string): Promise<Buffer | null> {
    const result = await this.db.query<{ content: Buffer }>('SELECT content FROM document_blobs WHERE document_id = $1 LIMIT 1', [documentId]);
  ...
  async upsertDocumentSource(input: { documentId: string; sourceUrl: string; normalizedSourceUrl: string; sourceHost: string; referrerUrl: string | null; anchorText: string | null; httpFilename: string | null; lastHttpStatus: number | null }): Promise<void> {
    await this.db.query(`INSERT INTO document_sources (document_id, source_url, normalized_source_url, source_host, referrer_url, anchor_text, http_filename, last_http_status) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (document_id, normalized_source_url) DO UPDATE SET source_url=EXCLUDED.source_url, referrer_url=COALESCE(EXCLUDED.referrer_url, document_sources.referrer_url), anchor_text=COALESCE(EXCLUDED.anchor_text, document_sources.anchor_text), http_filename=COALESCE(EXCLUDED.http_filename, document_sources.http_filename), last_http_status=EXCLUDED.last_http_status, last_seen_at=now()`, [input.documentId, input.sourceUrl, input.normalizedSourceUrl, input.sourceHost, input.referrerUrl, input.anchorText, input.httpFilename, input.lastHttpStatus]);
  }
  ...
  async getDocumentSources(documentId: string): Promise<Array<{ sourceUrl: string; sourceHost: string; anchorText: string | null; firstSeenAt: string; lastSeenAt: string }>> {
    const result = await this.db.query<{ source_url: string; source_host: string; anchor_text: string | null; first_seen_at: string; last_seen_at: string }>('SELECT source_url, source_host, anchor_text, first_seen_at, last_seen_at FROM document_sources WHERE document_id = $1 ORDER BY first_seen_at ASC', [documentId]);
  ...
  async searchDocuments(query: string, limit = 20): Promise<Array<DocumentRecord & { score: number; sourceCount: number }>> {
    ...
    const result = await this.db.query<DocumentRow & { score: string | number; source_count: string | number }>(`
      SELECT d.*,
        GREATEST(CASE WHEN d.document_number_normalized = regexp_replace($1, '\\s+', '', 'g') THEN 1.0 ELSE 0 END, similarity(COALESCE(d.preferred_title,''), $1), similarity(COALESCE(d.preferred_filename,''), $1), similarity(COALESCE(d.document_number_normalized,''), regexp_replace($1, '\\s+', '', 'g'))) AS score,
        COUNT(ds.id)::int AS source_count
      FROM documents d LEFT JOIN document_sources ds ON ds.document_id = d.id
      WHERE d.document_number_normalized = regexp_replace($1, '\\s+', '', 'g') OR d.preferred_title ILIKE '%' || $1 || '%' OR d.preferred_filename ILIKE '%' || $1 || '%' OR similarity(COALESCE(d.preferred_title,''), $1) > 0.2 OR similarity(COALESCE(d.document_number_normalized,''), regexp_replace($1, '\\s+', '', 'g')) > 0.25
